...
/**
 * The one no-data day we can assert for any security without a trading
 * calendar: the exchange is never open on a weekend. Exchange holidays are
 * also no-data days, but the API exposes no calendar to know which ones —
 * asserting those would be a guess, so only weekends are flagged here.
 */
