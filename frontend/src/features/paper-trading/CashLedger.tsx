...
export function CashLedger({ portfolioId }: CashLedgerProps) {
  ...
  if (isError) {
    ...
    return <ErrorCard>{readErrorText(error, t('portfolio.cashLedger.unreachable'))}</ErrorCard>;
  }
          ...
          <table className="bui-table">
            <thead>
              <tr>
                ...
                <th scope="col">{t('portfolio.cashLedger.columns.effectiveDate')}</th>
              </tr>
            </thead>
            <tbody>
              {isPending && !data
                ...
                : rows.map((transaction) => (
                    <tr key={transaction.transaction_id}>
                      <td>{t(`portfolio.cashLedger.types.${transaction.type}`)}</td>
                      <td className={`text-right tabular-nums ${toneClass(transaction.amount)}`}>
                        {formatSignedMoney(transaction.amount, locale)}
                      </td>
                      <td className="text-right tabular-nums">
                        {formatMoney(transaction.balance_after, locale)}
                      </td>
                      <td className="tabular-nums">{transaction.effective_date}</td>
                    </tr>
                  ))}
            </tbody>
          </table>
