...
async function fillCreateForm(user: ReturnType<typeof userEvent.setup>) {
  await user.type(
    await screen.findByRole("textbox", { name: t("portfolio.create.name") }),
    "Evaluation portfolio",
  );
  await user.type(
    screen.getByRole("spinbutton", {
      name: t("portfolio.create.startingCapital"),
    }),
    "1000000",
  );
...
describe("PortfolioPage", () => {
  it("offers the create form, not a table, when there are no portfolios", async () => {
    ...
    expect(
      await screen.findByText(t("portfolio.scope.empty")),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: t("portfolio.create.submit") }),
    ).toBeInTheDocument();
    expect(
      screen.queryByText(t("portfolio.positions.title")),
  ...
  it("renders summary and positions from the API", async () => {
    ...
    expect(
      await screen.findByText(positionsFixture[0].symbol),
    ).toBeInTheDocument();
    expect(
      screen.getByText(t("portfolio.positions.title")),
    ).toBeInTheDocument();
  ...
  it("does not render a previous portfolio's cash rows while switching to another portfolio", async () => {
    ...
    const summaryB = {
      ...summaryFixture,
      portfolio_id: portfolioB.portfolio_id,
    };
    ...
    server.use(
      ...
      http.get("*/portfolios/:portfolioId/summary", ({ params }) =>
        HttpResponse.json({
          data:
            params.portfolioId === portfolioB.portfolio_id
              ? summaryB
              : summaryFixture,
        }),
      ),
        ...
        async ({ params }) => {
          ...
          return HttpResponse.json({
            ...
            meta: {
              page: 1,
              page_size: 50,
              total: cashTransactionsFixture.length,
            },
          });
        },
      ),
    ...
    await user.click(
      screen.getByRole("button", {
        name: new RegExp(`${t("portfolio.selector.label")}$`),
      }),
    );
    await user.click(
      await screen.findByRole("option", { name: new RegExp(portfolioB.name) }),
    );
  ...
  it("shows the valuation date the API returned, not the one that was asked for", async () => {
    ...
    const user = userEvent.setup({ delay: null });
    await user.click(
      screen.getByRole("button", { name: t("portfolio.asOfLabel") }),
    );
  ...
  it("offers a way out of PRICE_UNAVAILABLE instead of an empty table", async () => {
    server.use(
      http.get("*/portfolios/:portfolioId/positions", () =>
        HttpResponse.json(
          errorBody(
            "PRICE_UNAVAILABLE",
            "No close for MISS.N0000 on 2025-01-10.",
          ),
          { status: 422 },
        ),
      ),
    ...
    expect(
      screen.queryByText(t("portfolio.positions.empty")),
    ).not.toBeInTheDocument();
  });
        ...
        HttpResponse.json(
          {
            error: {
              ...
              fields: [
                {
                  field: "as_of",
                  reason: "must fall between 2025-01-02 and 2025-01-10",
                },
              ],
              trace_id: "test-trace",
            },
    ...
    server.use(
      http.get("*/portfolios/:portfolioId/summary", ({ params }) =>
        params.portfolioId === staleId
          ? HttpResponse.json(errorBody("PORTFOLIO_NOT_FOUND", "Not found."), {
              status: 404,
            })
  ...
  it("marks a gain and a loss with more than colour", async () => {
    ...
    expect(
      within(gainRow as HTMLElement).getAllByText("▲").length,
    ).toBeGreaterThan(0);
    expect(
      within(gainRow as HTMLElement).queryByText("▼"),
    ).not.toBeInTheDocument();

    const lossRow = screen.getByText(positionsFixture[1].symbol).closest("tr");
    if (!lossRow) throw new Error("loss row not found");
    expect(
      within(lossRow as HTMLElement).getAllByText("▼").length,
    ).toBeGreaterThan(0);
    expect(
      within(lossRow as HTMLElement).queryByText("▲"),
    ).not.toBeInTheDocument();
  });
  ...
  it("surfaces an unreachable API rather than rendering an empty page", async () => {
    ...
    expect(
      await screen.findByText(t("portfolio.scope.unreachable")),
    ).toBeInTheDocument();
  });
});
...
describe("CreatePortfolioForm", () => {
  ...
  it("puts a 400 VALIDATION_FAILED on the field it names", async () => {
              ...
              fields: [
                {
                  ...
                  reason: "must not be less than 100000",
                },
              ],
              trace_id: "test-trace",
            },
    ...
    expect(
      await screen.findByText("must not be less than 100000"),
    ).toBeInTheDocument();
  });
    ...
    server.use(
      http.post("*/portfolios", ({ request }) => {
        ...
        return keys.length === 1
          ? HttpResponse.json(
              errorBody("IDEMPOTENCY_KEY_REUSED", "Key already used."),
              {
                status: 409,
              },
            )
