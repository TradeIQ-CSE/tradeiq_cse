...
import { filledOrderFixture, ordersListFixture, portfolioFixture, rejectedOrderFixture } from '../../test/fixtures/paper-trading';
...
function ordersListResponse(data = ordersListFixture) {
  return HttpResponse.json({ data, meta: { page: 1, page_size: 50, total: data.length } });
}
...
describe('OrdersTable', () => {
  ...
  it('shows a rejected order with its reason in the history table', async () => {
    server.use(http.get('*/portfolios/:portfolioId/orders', () => ordersListResponse()));
    ...
    const reasonNode = await screen.findByText(t('orders.codes.insufficientCash'));
  ...
  it('fetches an order once on expand and serves a re-expand from cache', async () => {
    ...
    renderWithProviders(<OrdersTable portfolioId={portfolioId} />, { queryClient });
    const user = userEvent.setup({ delay: null });

    await screen.findByText(filledOrderFixture.symbol);
    const expandButtons = screen.getAllByRole('button', { name: t('orders.expand') });
    await user.click(expandButtons[0]);

    expect(await screen.findByText(t('orders.detail.price'))).toBeInTheDocument();
    await waitFor(() => expect(detailCalls).toBe(1));

    await user.click(screen.getByRole('button', { name: t('orders.collapse') }));
    expect(screen.queryByText(t('orders.detail.price'))).not.toBeInTheDocument();

    // Both rows now read "Show details" again — re-expand the same (first) row.
    await user.click(screen.getAllByRole('button', { name: t('orders.expand') })[0]);
    expect(await screen.findByText(t('orders.detail.price'))).toBeInTheDocument();
  ...
  it('gives each status filter value its own cache entry', async () => {
    ...
    const { queryClient } = renderWithProviders(<OrdersTable portfolioId={portfolioId} />);
    const user = userEvent.setup({ delay: null });

    await screen.findByText(filledOrderFixture.symbol);

    await user.selectOptions(screen.getByLabelText(t('orders.filter.label')), 'filled');
    await waitFor(() =>
      expect(
        queryClient.getQueryCache().find({ queryKey: [...queryKeys.orders(portfolioId, 'filled', 1)], exact: true }),
      ).toBeTruthy(),
    );

    await user.selectOptions(screen.getByLabelText(t('orders.filter.label')), 'rejected');
    await waitFor(() =>
      expect(
        queryClient
          .getQueryCache()
          .find({ queryKey: [...queryKeys.orders(portfolioId, 'rejected', 1)], exact: true }),
      ).toBeTruthy(),
    );
