...
export function SymbolPicker({ value, onChange, disabled }: SymbolPickerProps) {
  ...
  useEffect(() => {
    const handle = setTimeout(() => setDebouncedValue(value), SEARCH_DEBOUNCE_MS);
  ...
  const { data, isFetching, isError, error } = useQuery({
    queryKey: ['securities', { search: trimmed, sort: 'symbol', page: 1, page_size: MAX_RESULTS }],
    queryFn: () =>
  ...
  function handleKeyDown(event: KeyboardEvent<HTMLInputElement>) {
    ...
    if (event.key === 'Enter') {
      ...
      if (!showDropdown || activeIndex < 0 || activeIndex >= results.length) return;
      event.preventDefault();
