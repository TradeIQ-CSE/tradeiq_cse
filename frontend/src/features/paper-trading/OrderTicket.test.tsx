...
function mockEstimate(
  body: Record<string, unknown> = { data: orderEstimateFixture },
  status = 200,
) {
  server.use(
...
async function fillTicket(user: ReturnType<typeof userEvent.setup>) {
  const symbolInput = screen.getByRole("combobox", {
    name: t("paperTrading.ticket.symbol"),
  });
  await user.clear(symbolInput);
  await user.type(symbolInput, "COMB.N0000");

  const quantityInput = screen.getByRole("spinbutton", {
    name: t("paperTrading.ticket.quantity"),
  });
...
async function preview(user: ReturnType<typeof userEvent.setup>) {
  await user.click(
    screen.getByRole("button", { name: t("paperTrading.ticket.preview") }),
  );
...
describe("OrderTicket", () => {
  ...
  it("shows the fees verbatim, fills the order, and invalidates every affected family", async () => {
    ...
    const queryClient = (() => {
      const { queryClient: client } = renderWithProviders(
        <OrderTicket portfolioId={portfolioId} />,
      );
    ...
    for (const fee of orderEstimateFixture.fees) {
      expect(
        screen.getByText(formatMoney(fee.amount, locale)),
      ).toBeInTheDocument();
    }
    expect(
      screen.getByText(formatMoney(orderEstimateFixture.fee_total, locale)),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        formatMoney(orderEstimateFixture.gross_consideration, locale),
      ),
    ).toBeInTheDocument();
    ...
    await waitFor(() => {
      ...
      expect(isInvalidated(queryKeys.cashTransactions(portfolioId, 1))).toBe(
        true,
      );
      expect(isInvalidated(queryKeys.orders(portfolioId, undefined, 1))).toBe(
        true,
      );
  ...
  it("renders a rejected order as a warning, not an error, and invalidates only orders", async () => {
    ...
    expect(
      screen.getByText(t("orders.codes.insufficientCash")),
    ).toBeInTheDocument();

    const positionsEntry = queryClient
      .getQueryCache()
      .find({ queryKey: [...queryKeys.positions(portfolioId)], exact: true });
    const ordersEntry = queryClient.getQueryCache().find({
      queryKey: [...queryKeys.orders(portfolioId, undefined, 1)],
      exact: true,
    });
  ...
  it("renders a 422 estimate error identically to the equivalent 201 rejection", async () => {
    ...
    expect(
      await screen.findByText(t("orders.codes.insufficientCash")),
    ).toBeInTheDocument();
  ...
  it("retries a 503 with the same Idempotency-Key", async () => {
    ...
    await user.click(confirmButton());
    const banner = await screen.findByText(
      t("orders.codes.dependencyUnavailable"),
    );
  ...
  it("disables Confirm the moment symbol, side or quantity changes after a preview", async () => {
    ...
    // Side.
    await user.click(
      screen.getByRole("radio", { name: t("paperTrading.ticket.sides.sell") }),
    );
    expect(confirmButton()).toBeDisabled();
  });
