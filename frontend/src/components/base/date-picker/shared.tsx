...
export function DayCell(props: CalendarCellRenderProps & { isRange: boolean }) {
  const { date, formattedDate, isSelected, isSelectionStart, isSelectionEnd, isHovered, isFocusVisible, isDisabled, isUnavailable, isOutsideMonth, isRange } = props;
  ...
  return (
    <div className="relative size-8">
      ...
      >
        ...
        <span
          className={cx(
            "relative text-body-medium text-text-primary",
            (isDisabled || isUnavailable) && "text-text-tertiary",
            // Distinct from a plain out-of-coverage day: struck through
            // rather than just greyed, so "closed that day" (e.g. a
            // weekend) reads differently from "outside this security's
            // history".
            isUnavailable && "line-through decoration-1",
          )}
        >
          {formattedDate}
        </span>
